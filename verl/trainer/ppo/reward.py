...
def get_custom_eval_reward_fn(config):
    """Load and return a custom reward function from external file.
    ...
    reward_fn_config = config.get("custom_reward_function") or {}
    file_path = reward_fn_config.get("eval_path")
    if not file_path:
        return None
    ...
    function_name = reward_fn_config.get("eval_name")
    if not hasattr(module, function_name):
        raise AttributeError(f"Reward function '{function_name}' not found in '{file_path}'.")
...
def load_eval_reward_manager(config, tokenizer, num_examine, **reward_kwargs):
    """
    Load and initialize a reward manager based on the configuration.
    ...
    # registered via `verl.workers.reward_manager.register`
    # By default reward_manager is set to naive (NaiveRewardManager)
    reward_manager_name = config.reward_model.get("eval_reward_manager", "naive")
    reward_manager_cls = get_reward_manager_cls(reward_manager_name)

    # Try to get a custom reward function based on the configuration
    compute_score = get_custom_eval_reward_fn(config)
    final_compute_score = compute_score
