...
def get_process_score(solution_str: str,
    ...
    batch_message = generate_proccess_score_data(solution_str, split_step_num)
    if isinstance(batch_message, list) and len(batch_message) > 1:
        batch_message = batch_message[-1:]
    process_scores_raw = client.auto(batch_message, temperature=temperature, max_tokens=max_tokens, concurrency=concurrency)  # concurrency 任意

    # 讲输出的字符串解析为 python 列表
    process_scores = parse_last_line_binary(process_scores_raw)

    return process_scores


@register("modified_dapo")
class ModifiedDAPORewardManager:
    """The reward manager."""
